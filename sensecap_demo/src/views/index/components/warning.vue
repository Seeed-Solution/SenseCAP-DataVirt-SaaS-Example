      ...
      <div class="per-parent">
        <div class="per-all" :class="{'top-transition': isScroll}">
          <div class="per" v-for="(item, index) in allData" :key="index + item.measure_id + item.time + item.dev_eui + 'warn'">
            ...
            <div class="per-bot">
              <span class="name" :class="{'red': item.status == 2, 'yellow': item.status == 0}">{{item.status == 0 ? $t('localization').value_lit : $t('localization').value_big}} <span
                  style="font-size: 0.1rem; color: #fff; line-height: 0.08rem;">({{$t('localization.measurements' + item.measure_id)}})</span></span>
